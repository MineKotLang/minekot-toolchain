...
| Indentation and spacing        | Partial       | Readability-safe ktlint spacing rules remain enabled; `Indentation` and `NoConsecutiveBlankLines` are disabled. Standalone comments align with neighboring code. Generated build output is excluded from handwritten-source style findings. Complete guide coverage is not proven.          |
...
| Intent-based value extraction  | Partial       | `MagicNumber` checks suspicious literals while excluding common structural values, named properties/constants, annotations, and direct named call arguments whose argument names carry the meaning. Its positive/negative matrix includes exact named-versus-positional call regressions; broader extraction intent still requires judgment.          |
...
| Comment necessity              | Not automated | Safe marker-spacing and standalone-indentation findings auto-correct during Detekt detection by default; necessity and content require human review.                                                                                                                                       |
...
| Plugin declarations and block order  | Partial       | Imperative plugin application is rejected. Plugin-before-build-configuration ordering applies to `build.gradle.kts`; `settings.gradle.kts` follows its distinct `pluginManagement` layout. Complete top-level ordering is unfinished. |
...
- `check` depends only on deterministic verification; no human approval receipt or semantic-review file is required. It runs plain build-script Detekt plus compilation-aware `detektMain` and `detektTest` tasks. Every regular Detekt task excludes producer-owned generated sources under each project build directory while compilation still validates those outputs. Staged formatter diagnostics for Kotlin sources and `.mkot.kts` resources use paths that resolve to the original root-project or subproject files, never disposable `build/tmp` mirrors.
