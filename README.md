...
Detekt auto-correction is enabled by default. Safe configured corrections are applied when Detekt detects them; corrected comment marker spacing and standalone-comment alignment do not fail that run. Set `minekotToolchain.lint.autoCorrect=false` to make Detekt report-only; `mineKotFormat` remains the staged, compilation-verified bulk formatter.
